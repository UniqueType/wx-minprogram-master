...
function getDistricts(that, event) {
    ...
    query(adcode, function (data) {
        ...
        //此处二级联动  设置地址缓存，在index页面获取缓存
         wx.setStorage({
           key: 'address',
           data: districts.selectedProvince+districts.selectedCity,
           success: function(res){
             // success设置缓存成功！
           },
         })
...
function finish(that, event) {
    ...
    //三级联动  设置地址缓存，在index页面获取缓存
     wx.setStorage({
           key: 'address',
           data: districts.selectedProvince+districts.selectedCity+districts.selectedDistrict,
           success: function(res){
             // success设置缓存成功！
           },
         })
